...
// Mark 'to_pay' orders as read
$update = $conn->prepare("UPDATE orders SET is_read = 1 WHERE user_id = ? AND status = 'to_pay'");
$update->bind_param("i", $user_id);
$update->execute();

// Get all orders with status 'to_pay'
$orderStmt = $conn->prepare("SELECT * FROM orders WHERE user_id = ? AND status = 'to_pay' ORDER BY created_at DESC");
...
<div class="container">
  ...
  <?php if ($orderResult->num_rows === 0): ?>
    <div class="empty-msg">You have no pending "To Pay orders at the moment.</div>
  <?php else: ?>
