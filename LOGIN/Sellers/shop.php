...
$sql = "SELECT p.*, s.shop_name, s.seller_id 
        FROM products p 
...
if (!empty($search)) {
    $stmt->bind_param("is", $seller_id, $searchParam);
} else {
...
$joined = date('F Y', strtotime($seller['created_at'] ?? 'now'));

$logo_path = $seller['logo_path'] ?? 'images/default-store.jpg';
...
<head>
  <meta charset="UTF-8">
  <title><?= htmlspecialchars($shop_name) ?> - Seller Shop</title>
...
<div class="banner">ShopSmart Seller Store</div>

<div class="seller-profile">
  <div class="profile-left">
<img src="/LOGIN/<?= htmlspecialchars($logo_path) ?>" alt="Shop Logo">
<div class="shop-info">
  <h2><?= htmlspecialchars($shop_name) ?></h2>
  <p>Active now · Joined <?= $joined ?></p>
  <p><strong>Category:</strong> <?= htmlspecialchars($seller['category']) ?></p>
  <p><strong>Address:</strong> <?= htmlspecialchars($seller['shop_address']) ?></p>
  <p><strong>Contact:</strong> <?= htmlspecialchars($seller['shop_phone']) ?></p>
</div>
  </div>
  <div class="stats">
<div>Products: <?= $result->num_rows ?></div>
    <div>Followers: <?= rand(1000, 9999) ?></div>
    <div>Rating: <?= number_format(rand(40, 50) / 10, 1) ?> ⭐</div>
  </div>
</div>
...
<div style="margin: 30px 40px;">
  <input type="text" id="liveSearchInput" placeholder="Search products..." 
         style="padding: 8px 10px; width: 250px; border: 1px solid #ccc; border-radius: 20px;">
</div>
...
'<?= ($p['has_sizes'] === '1' || strtolower($p['has_sizes']) === 'yes') ? 'yes' : 'no' ?>',
<?= (int)$p['stock_s'] ?>,
<?= (int)$p['stock_m'] ?>,
<?= (int)$p['stock_l'] ?>,
<?= (int)$p['stock_xl'] ?>,
...
document.getElementById('modalForm').addEventListener('submit', function(e) {
  ...
  fetch('/LOGIN/Ecommerce/add-to-cart.php', {
    method: 'POST',
    ...
    document.querySelectorAll('.save-to-saved-form').forEach(form => {
      ...
      form.addEventListener('submit', function(e) {
        ...
        fetch('/LOGIN/Ecommerce/saved-item.php', {
          method: 'POST',
          headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
  ...
  const sellerId = <?= (int)$seller_id ?>;

  searchInput.addEventListener('input', function () {
    const query = this.value.trim();

    fetch(`shop.php?seller=${sellerId}&q=${encodeURIComponent(query)}`)
      .then(res => res.text())
      .then(html => {
        // Extract the new .products content from the response
        const temp = document.createElement('div');
        temp.innerHTML = html;
        const newProducts = temp.querySelector('.products');

        if (newProducts) {
          document.querySelector('.products').innerHTML = newProducts.innerHTML;

          // Rebind event listeners (Add to Cart, Save, Modal)
          rebindItemCardEvents();
        }
      });
  });

  function rebindItemCardEvents() {
    // Rebind all your product card logic here
    document.querySelectorAll('.save-to-saved-form').forEach(form => {
      form.addEventListener('click', e => e.stopPropagation());
      form.addEventListener('submit', function(e) {
        e.preventDefault();
    ...
    // You may also need to rebind openModal() triggers if they rely on event listeners instead of inline onclick.
  }
</script>
